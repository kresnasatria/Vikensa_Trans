...
<div class="lg:pl-[285px]">
    ...
    <header class="sticky top-0 z-30 flex h-20 items-center border-b border-slate-200 bg-white/90 px-5 backdrop-blur-xl sm:px-7 lg:px-10">
        ...
        <div class="flex w-full items-center justify-between">
            <div class="flex items-center gap-4">
                <button type="button" @click="sidebarOpen = true" class="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 text-slate-600 lg:hidden">
                ...
                <div>
                    ...
                    <h2 class="text-lg font-black text-slate-900">Order Masuk</h2>
                </div>
            </div>
    ...
    <main class="px-5 py-8 sm:px-7 lg:px-10">
        ...
        <div class="mx-auto max-w-7xl">
            ...
            {{-- SUCCESS --}}
            @if(session('success'))
                <div x-data="{ show: true }" x-show="show" class="mb-6 flex items-center justify-between gap-4 rounded-xl border border-emerald-200 bg-emerald-50 px-5 py-4">
            ...
            <div class="flex flex-col justify-between gap-5 md:flex-row md:items-end">
                <div>
                    <p class="text-xs font-bold uppercase tracking-[.15em] text-sky-600">Manajemen Pesanan</p>
                    <h1 class="mt-2 text-3xl font-black tracking-tight text-slate-950">Pesanan yang masuk</h1>
                    <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
                        Cek data pemesan, perjalanan dan status pembayaran dari seluruh order VikensaTrans.
                    </p>
                </div>
                ...
                @if($unreadOrders > 0)
                    <form action="{{ route('admin.orders.markRead') }}" method="POST">
                        @csrf
                        <button type="submit" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-xs font-bold text-slate-700 transition hover:border-sky-300 hover:text-sky-600">
            ...
            <div class="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div class="rounded-2xl border border-slate-200 bg-white p-5">
                    <p class="text-xs font-semibold text-slate-400">Total Order</p>
                    <p class="mt-2 text-2xl font-black text-slate-900">{{ $totalOrders }}</p>
                </div>
                <div class="rounded-2xl border border-slate-200 bg-white p-5">
                    <p class="text-xs font-semibold text-slate-400">Order Baru</p>
                    <p class="mt-2 text-2xl font-black text-sky-600">{{ $unreadOrders }}</p>
                </div>
                <div class="rounded-2xl border border-slate-200 bg-white p-5">
                    <p class="text-xs font-semibold text-slate-400">Pending</p>
                    <p class="mt-2 text-2xl font-black text-amber-500">{{ $pendingOrders }}</p>
                </div>
                <div class="rounded-2xl border border-slate-200 bg-white p-5">
                    <p class="text-xs font-semibold text-slate-400">Lunas</p>
                    <p class="mt-2 text-2xl font-black text-emerald-600">{{ $paidOrders }}</p>
                </div>
            </div>


            {{-- FILTER --}}
            <div class="mt-8 flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-4 md:flex-row md:items-center md:justify-between">
                <div class="relative w-full md:max-w-sm">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400">
                        <circle cx="11" cy="11" r="7"/>
                        <path d="m20 20-3.5-3.5"/>
                    </svg>
                    <input type="text" id="orderSearch" placeholder="Cari kode, nama, nomor HP..." class="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 pl-11 pr-4 text-sm outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-500/10">
                </div>

                <div class="flex gap-2 overflow-x-auto">
                    <button type="button" data-filter="all" class="order-filter whitespace-nowrap rounded-lg bg-slate-950 px-4 py-2.5 text-xs font-bold text-white">Semua</button>
                    <button type="button" data-filter="unread" class="order-filter whitespace-nowrap rounded-lg bg-slate-100 px-4 py-2.5 text-xs font-bold text-slate-600">Baru</button>
                    <button type="button" data-filter="pending" class="order-filter whitespace-nowrap rounded-lg bg-slate-100 px-4 py-2.5 text-xs font-bold text-slate-600">Pending</button>
                    <button type="button" data-filter="paid" class="order-filter whitespace-nowrap rounded-lg bg-slate-100 px-4 py-2.5 text-xs font-bold text-slate-600">Lunas</button>
                </div>
            </div>
            ...
            <div class="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white">
                    ...
                    <table class="w-full min-w-[1180px]">
                        <thead class="bg-slate-50">
                            <tr class="border-b border-slate-200 text-left">
                                <th class="px-5 py-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Booking</th>
                                <th class="px-5 py-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Pemesan</th>
                                <th class="px-5 py-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Perjalanan</th>
                                <th class="px-5 py-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Armada</th>
                                <th class="px-5 py-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Harga</th>
                                <th class="px-5 py-4 text-[10px] font-black uppercase tracking-wider text-slate-400">Status</th>
                                <th class="px-5 py-4 text-right text-[10px] font-black uppercase tracking-wider text-slate-400">Aksi</th>
                            </tr>
                        </thead>
                        ...
                        <tbody id="orderTableBody">
                            @forelse($orders as $order)
                                @php
                                    $departure = $order->custom_departure_time ? \Carbon\Carbon::parse($order->custom_departure_time) : null;
                                    $arrival = $order->custom_arrival_time ? \Carbon\Carbon::parse($order->custom_arrival_time) : null;
                                    $isUnread = !(bool) $order->is_read;
                                @endphp

                                <tr
                                    class="order-row border-b border-slate-100 last:border-0 {{ $isUnread ? 'bg-sky-50/40' : 'bg-white' }} hover:bg-slate-50"
                                    data-status="{{ $order->payment_status }}"
                                    data-read="{{ $isUnread ? 'unread' : 'read' }}"
                                    data-search="{{ strtolower(($order->booking_code ?? '') . ' ' . ($order->booker_name ?? '') . ' ' . ($order->phone_number ?? '') . ' ' . ($order->custom_origin ?? '') . ' ' . ($order->custom_destination ?? '')) }}"
                                >
                                    {{-- BOOKING --}}
                                    <td class="px-5 py-5 align-top">
                                        <div class="flex items-start gap-3">
                                            @if($isUnread)
                                                <span class="mt-1.5 h-2.5 w-2.5 shrink-0 rounded-full bg-sky-500" title="Pesanan baru"></span>
                                            @else
                                                <span class="mt-1.5 h-2.5 w-2.5 shrink-0 rounded-full bg-slate-200"></span>
                                            @endif
                                            <div>
                                                <p class="font-black text-sky-600">{{ $order->booking_code }}</p>
                                                <p class="mt-1 text-xs font-semibold text-slate-500">{{ $order->created_at->format('d M Y') }}</p>
                                                <p class="mt-0.5 text-[11px] text-slate-400">{{ $order->created_at->format('H:i') }} WIB</p>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    {{-- PEMESAN --}}
                                    <td class="px-5 py-5 align-top">
                                        <p class="font-bold text-slate-900">{{ $order->booker_name ?? ($order->user->name ?? 'Pengguna') }}</p>
                                        <p class="mt-1 text-xs font-semibold text-slate-500">{{ $order->phone_number ?? '-' }}</p>
                                        <p class="mt-2 max-w-[220px] truncate text-xs text-slate-400" title="{{ $order->pickup_address }}">{{ $order->pickup_address ?? 'Alamat jemput belum ada' }}</p>
                                    </td>

                                    {{-- PERJALANAN --}}
                                    <td class="px-5 py-5 align-top">
                                        <div class="flex items-center gap-2">
                                            <span class="font-bold text-slate-800">{{ $order->custom_origin ?? '-' }}</span>
                                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="h-4 w-4 shrink-0 text-sky-500"><path d="M5 12h14"/><path d="m13 6 6 6-6 6"/></svg>
                                            <span class="font-bold text-slate-800">{{ $order->custom_destination ?? '-' }}</span>
                                        </div>
                                        @if($departure)
                                            <p class="mt-2 text-xs text-slate-500">{{ $departure->format('d M Y, H:i') }} WIB</p>
                                        @endif
                                        @if($arrival)
                                            <p class="mt-1 text-[11px] text-slate-400">Selesai: {{ $arrival->format('d M Y, H:i') }} WIB</p>
                                        @endif
                                    </td>

                                    {{-- ARMADA --}}
                                    <td class="px-5 py-5 align-top">
                                        <p class="font-bold text-slate-800">{{ $order->schedule?->shuttle?->name ?? 'Armada Charter' }}</p>
                                        @if($order->schedule?->shuttle?->license_plate)
                                            <p class="mt-1 text-xs font-semibold uppercase text-slate-400">{{ $order->schedule->shuttle->license_plate }}</p>
                                        @endif
                                    </td>

                                    {{-- HARGA --}}
                                    <td class="px-5 py-5 align-top">
                                        <p class="whitespace-nowrap font-black text-slate-900">Rp {{ number_format($order->total_price, 0, ',', '.') }}</p>
                                    </td>

                                    {{-- STATUS --}}
                                    <td class="px-5 py-5 align-top">
                                        @if($order->payment_status === 'paid')
                                            <span class="inline-flex items-center gap-1.5 rounded-full bg-emerald-100 px-3 py-1.5 text-[10px] font-black uppercase text-emerald-700">
                                                <span class="h-1.5 w-1.5 rounded-full bg-emerald-500"></span>Lunas
                                            </span>
                                        @elseif($order->payment_status === 'cancelled')
                                            <span class="inline-flex items-center gap-1.5 rounded-full bg-red-100 px-3 py-1.5 text-[10px] font-black uppercase text-red-600">
                                                <span class="h-1.5 w-1.5 rounded-full bg-red-500"></span>Dibatalkan
                                            </span>
                                        @else
                                            <span class="inline-flex items-center gap-1.5 rounded-full bg-amber-100 px-3 py-1.5 text-[10px] font-black uppercase text-amber-700">
                                    ...
                                    <td class="px-5 py-5 align-top">
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            {{-- RECEIPT --}}
                                            @if($order->payment_status === 'paid')
                                                <a href="{{ route('booking.receipt', $order->id) }}" target="_blank" title="Kwitansi" class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-sky-50 text-sky-600 transition hover:bg-sky-500 hover:text-white">
                                ...
                                <tr>
                                    <td colspan="7" class="px-6 py-16 text-center">
                                        <div class="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-slate-100 text-slate-400">
                                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" class="h-6 w-6"><path d="M5 4h14v16l-3-2-4 2-4-2-3 2Z"/><path d="M8 8h8M8 12h6"/></svg>
                                        </div>
                                        <p class="mt-4 font-bold text-slate-800">Belum ada order masuk</p>
                                        <p class="mt-1 text-sm text-slate-400">Pesanan pelanggan akan muncul di sini.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                {{-- NO SEARCH RESULT --}}
                <div id="emptySearch" class="hidden px-6 py-14 text-center">
                    <p class="font-bold text-slate-700">Pesanan tidak ditemukan</p>
                    <p class="mt-1 text-sm text-slate-400">Coba gunakan kata pencarian atau filter yang lain.</p>
                </div>
            </div>
