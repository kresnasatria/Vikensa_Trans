...
@php
    ...
    $totalStops = $routes->sum(function ($route) {
        $destinations = is_string($route->destination)
            ...
            : $route->destination;
        return is_array($destinations) ? count($destinations) : 1;
    });
@endphp
...
{{-- MOBILE OVERLAY --}}
{{-- ========================================================= --}}
<div x-show="sidebarOpen" x-cloak x-transition.opacity @click="sidebarOpen = false" class="fixed inset-0 z-40 bg-slate-950/60 lg:hidden"></div>
    ...
    <header class="sticky top-0 z-30 flex h-20 items-center border-b border-slate-200 bg-white/90 px-5 backdrop-blur-xl sm:px-7 lg:px-10">
        <div class="flex w-full items-center justify-between gap-5">
            <div class="flex items-center gap-4">
                <button type="button" @click="sidebarOpen = true" class="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 text-slate-600 lg:hidden transition hover:bg-slate-50">
                ...
                <div>
                    ...
                    <h2 class="text-lg font-black text-slate-950">Manajemen Rute</h2>
                </div>
            </div>
    ...
    <main class="px-5 py-8 sm:px-7 lg:px-10">
        <div class="mx-auto max-w-7xl">
            ...
            <div class="flex flex-col justify-between gap-5 md:flex-row md:items-end">
                <div>
                    <p class="text-xs font-bold uppercase tracking-[.15em] text-sky-600">Data Perjalanan</p>
                    <h1 class="mt-2 text-3xl font-black tracking-tight text-slate-950">Manajemen Rute</h1>
                    <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Tambahkan kota asal dan beberapa kota tujuan atau titik transit yang bisa dipilih pelanggan saat melakukan pemesanan.</p>
                </div>
                <div class="flex gap-3">
                    <div class="rounded-xl border border-slate-200 bg-white px-5 py-3">
                        <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Total Rute</p>
                        <p class="mt-1 text-xl font-black text-slate-900">{{ $totalRoutes }}</p>
                    </div>
                    <div class="rounded-xl border border-slate-200 bg-white px-5 py-3">
                        <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Titik Tujuan</p>
                        <p class="mt-1 text-xl font-black text-sky-600">{{ $totalStops }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-8 grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
                ...
                <section class="rounded-2xl border border-slate-200 bg-white">
                    <div class="border-b border-slate-100 px-6 py-5">
                        <h2 class="text-lg font-black text-slate-900">Tambah Rute Baru</h2>
                        <p class="mt-1 text-sm text-slate-500">Satu kota asal dapat memiliki beberapa tujuan atau titik transit.</p>
                    </div>

                    <form action="{{ route('admin.route.store') }}" method="POST" x-data="{ destinations: {{ Illuminate\Support\Js::from(old('destinations', [''])) }} }" class="p-6">
                        @csrf
                        <div class="grid gap-6 lg:grid-cols-2">

                            {{-- ORIGIN --}}
                            <div>
                                <label for="origin" class="block text-sm font-bold text-slate-700">Kota Asal <span class="text-red-500">*</span></label>
                                <p class="mt-1 text-xs text-slate-400">Titik awal perjalanan.</p>
                                <div class="relative mt-3">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-sky-500"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="3"/></svg>
                                    <input id="origin" type="text" name="origin" value="{{ old('origin') }}" placeholder="Contoh: Bandung" maxlength="255" required class="h-13 w-full rounded-xl border border-slate-200 bg-slate-50 py-3.5 pl-12 pr-4 text-sm font-semibold text-slate-800 outline-none transition placeholder:font-normal placeholder:text-slate-400 focus:border-sky-500 focus:bg-white focus:ring-4 focus:ring-sky-500/10">
                                </div>
                                @error('origin') <p class="mt-2 text-xs font-semibold text-red-500">{{ $message }}</p> @enderror
                            </div>
                            ...
                            <div>
                                <div class="flex items-start justify-between gap-4">
                                    <div>
                                        <label class="block text-sm font-bold text-slate-700">Kota Tujuan / Transit <span class="text-red-500">*</span></label>
                                        <p class="mt-1 text-xs text-slate-400">Urutkan sesuai perjalanan.</p>
                                    </div>
                                    <button type="button" @click="destinations.push('')" class="shrink-0 rounded-lg bg-sky-50 px-3 py-2 text-xs font-bold text-sky-600 transition hover:bg-sky-100">+ Tambah</button>
                                </div>
                                ...
                                <div class="mt-3 space-y-3">
                                    <template x-for="(destination, index) in destinations" :key="index">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <input type="text" name="destinations[]" x-model="destinations[index]" placeholder="Contoh: Jakarta" maxlength="255" required class="h-12 min-w-0 flex-1 rounded-xl border border-slate-200 bg-slate-50 px-4 text-sm font-semibold text-slate-800 outline-none transition placeholder:font-normal placeholder:text-slate-400 focus:border-sky-500 focus:bg-white focus:ring-4 focus:ring-sky-500/10">
                                            <button type="button" x-show="destinations.length > 1" @click="destinations.splice(index, 1)" title="Hapus titik" class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-red-50 text-red-500 transition hover:bg-red-500 hover:text-white">
                                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="h-4 w-4"><path d="M6 6l12 12M18 6 6 18"/></svg>
                                            </button>
                                        </div>
                                    </template>
                                </div>
                                @error('destinations') <p class="mt-2 text-xs font-semibold text-red-500">{{ $message }}</p> @enderror
                                @error('destinations.*') <p class="mt-2 text-xs font-semibold text-red-500">{{ $message }}</p> @enderror
                            </div>
                        </div>
                        ...
                        <div class="mt-6 border-t border-slate-100 pt-6">
                            <label for="biaya" class="block text-sm font-bold text-slate-700">Biaya Rute (Opsional / Rahasia)</label>
                            <p class="mt-1 text-xs text-slate-400">Nominal ini akan dikalkulasikan ke total harga saat user booking, namun detail angkanya tidak akan ditampilkan.</p>
                            <div class="relative mt-3">
                                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-sm font-bold text-slate-400">Rp</span>
                                <input id="biaya" type="number" name="biaya" value="{{ old('biaya', 0) }}" placeholder="Contoh: 150000" min="0" required class="h-13 w-full rounded-xl border border-slate-200 bg-slate-50 py-3.5 pl-12 pr-4 text-sm font-semibold text-slate-800 outline-none transition placeholder:font-normal placeholder:text-slate-400 focus:border-sky-500 focus:bg-white focus:ring-4 focus:ring-sky-500/10">
                            </div>
                            @error('biaya') <p class="mt-2 text-xs font-semibold text-red-500">{{ $message }}</p> @enderror
                        </div>

                        {{-- ACTION --}}
                        <div class="mt-7 flex justify-end border-t border-slate-100 pt-5">
                            <button type="submit" class="inline-flex items-center gap-2 rounded-xl bg-sky-500 px-6 py-3 text-sm font-bold text-white shadow-lg shadow-sky-500/15 transition hover:bg-sky-600">
                ...
                <aside class="rounded-2xl border border-slate-200 bg-white p-6">
                    <h3 class="text-base font-black text-slate-900">Contoh penyusunan rute</h3>
                    <p class="mt-2 text-sm leading-6 text-slate-500">Kalau perjalanan melewati beberapa kota, masukkan sesuai urutan perjalanan.</p>
                    <div class="mt-6 space-y-0">
                        <div class="flex items-center gap-3">
                            <span class="flex h-8 w-8 items-center justify-center rounded-full bg-sky-500 text-xs font-black text-white">A</span>
                            <div>
                                <p class="text-xs text-slate-400">Kota asal</p>
                                <p class="text-sm font-bold text-slate-800">Bandung</p>
                            </div>
                        </div>
                        <div class="ml-[15px] h-8 w-px bg-slate-200"></div>
                        <div class="flex items-center gap-3">
                            <span class="flex h-8 w-8 items-center justify-center rounded-full border-2 border-sky-300 bg-white text-xs font-black text-sky-600">1</span>
                            <div>
                                <p class="text-xs text-slate-400">Transit</p>
                                <p class="text-sm font-bold text-slate-800">Bekasi</p>
                            </div>
                        </div>
                        <div class="ml-[15px] h-8 w-px bg-slate-200"></div>
                        <div class="flex items-center gap-3">
                            <span class="flex h-8 w-8 items-center justify-center rounded-full bg-slate-950 text-xs font-black text-white">2</span>
                            <div>
                                <p class="text-xs text-slate-400">Tujuan akhir</p>
                                <p class="text-sm font-bold text-slate-800">Jakarta</p>
                            </div>
                        </div>
                    </div>
                    <div class="mt-6 rounded-xl bg-slate-50 p-4">
                        <p class="text-xs leading-6 text-slate-500">Rute yang disimpan di sini akan menjadi pilihan kota pada form booking pelanggan.</p>
                    </div>
                </aside>
            </div>
            ...
            <section class="mt-8 overflow-hidden rounded-2xl border border-slate-200 bg-white">
                <div class="flex flex-col justify-between gap-4 border-b border-slate-100 px-6 py-5 sm:flex-row sm:items-center">
                    <div>
                        <h2 class="text-lg font-black text-slate-900">Daftar Rute</h2>
                        <p class="mt-1 text-sm text-slate-500">{{ $totalRoutes }} rute sudah terdaftar.</p>
                    </div>
                    <div class="relative w-full sm:w-72">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/></svg>
                        <input type="text" id="liveSearchInput" placeholder="Cari kota..." class="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 pl-11 pr-4 text-sm text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-sky-500 focus:bg-white focus:ring-4 focus:ring-sky-500/10">
                    </div>
                </div>
                    ...
                    <table class="w-full min-w-[760px]">
                        <thead>
                            <tr class="border-b border-slate-200 bg-slate-50 text-left">
                                <th class="w-20 px-6 py-4 text-left text-[10px] font-black uppercase tracking-wider text-slate-400">No.</th>
                                <th class="px-6 py-4 text-left text-[10px] font-black uppercase tracking-wider text-slate-400">Rute Perjalanan</th>
                                <th class="w-36 px-6 py-4 text-center text-[10px] font-black uppercase tracking-wider text-slate-400">Titik</th>
                                <th class="w-32 px-6 py-4 text-right text-[10px] font-black uppercase tracking-wider text-slate-400">Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($routes as $rute)
                                @php
                                    ...
                                    if (!is_array($destinations)) { $destinations = [$rute->destination]; }
                                    $routeSearch = strtolower(trim($rute->origin . ' ' . implode(' ', $destinations)));
                                @endphp
                                <tr class="route-row border-b border-slate-100 transition last:border-0 hover:bg-slate-50/70" data-search="{{ $routeSearch }}">
                                    <td class="px-6 py-5 align-top">
                                        <span class="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-100 text-xs font-black text-slate-500">{{ $loop->iteration }}</span>
                                    </td>
                                    <td class="route-text px-6 py-5">
                                        <div class="flex flex-wrap items-center gap-2">
                                            <span class="inline-flex items-center gap-2 rounded-lg bg-sky-50 px-3 py-2 text-sm font-black text-sky-700">
                                                <span class="h-2 w-2 rounded-full bg-sky-500"></span>{{ $rute->origin }}
                                            </span>
                                            @foreach($destinations as $index => $destination)
                                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="h-4 w-4 shrink-0 text-slate-300"><path d="M5 12h14"/><path d="m13 6 6 6-6 6"/></svg>
                                                <span class="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-700">{{ $destination }}</span>
                                            @endforeach
                                        </div>
                                        <div class="mt-2 flex items-center gap-2.5 text-xs">
                                            <span class="text-slate-400">Dibuat {{ $rute->created_at->format('d M Y') }}</span>
                                            <span class="h-1 w-1 rounded-full bg-slate-300"></span>
                                            <span class="font-bold text-emerald-500">Biaya: Rp {{ number_format($rute->biaya ?? 0, 0, ',', '.') }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-5 text-center align-top">
                                        <span class="inline-flex rounded-full bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-600">{{ count($destinations) }} tujuan</span>
                                    </td>
                                    <td class="px-6 py-5 text-right align-top">
                                        <div class="flex items-center justify-end gap-2">
                                            <a href="{{ route('admin.route.edit', $rute->id) }}" title="Edit Rute" class="inline-flex h-[34px] w-[34px] items-center justify-center rounded-lg bg-amber-50 text-amber-500 transition hover:bg-amber-500 hover:text-white">
                                            ...
                                            <form action="{{ route('admin.route.destroy', $rute->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus rute {{ addslashes($rute->origin) }} ini?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="inline-flex items-center gap-2 rounded-lg bg-red-50 px-3 py-2 text-xs font-bold text-red-500 transition hover:bg-red-500 hover:text-white">
                                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="h-4 w-4"><path d="M3 6h18"/><path d="M8 6V4h8v2"/><path d="m19 6-1 14H6L5 6"/><path d="M10 11v5M14 11v5"/></svg> Hapus
                                                </button>
                                            </form>
                                        </div>
                                    </td>
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-16 text-center">
                                        <div class="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-slate-100 text-slate-400">
                                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" class="h-6 w-6"><circle cx="6" cy="18" r="2"/><circle cx="18" cy="6" r="2"/><path d="M7.5 16.5c2-4 7-4 9-8.5"/></svg>
                                        </div>
                                        <p class="mt-4 font-bold text-slate-800">Belum ada rute</p>
                                        <p class="mt-1 text-sm text-slate-400">Tambahkan rute perjalanan pertama menggunakan form di atas.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                <div class="divide-y divide-slate-100 md:hidden">
                    @forelse($routes as $rute)
                        @php
                            ...
                            $routeSearch = strtolower(trim($rute->origin . ' ' . implode(' ', $destinations)));
                        @endphp
                        <div class="route-mobile p-5" data-search="{{ $routeSearch }}">
                            <div class="flex items-start justify-between gap-4">
                                <div>
                                    <p class="text-[10px] font-black uppercase tracking-wider text-slate-400">Kota Asal</p>
                                    <p class="mt-1 text-base font-black text-sky-700">{{ $rute->origin }}</p>
                                </div>
                                <span class="shrink-0 rounded-full bg-slate-100 px-3 py-1.5 text-[10px] font-bold text-slate-600">{{ count($destinations) }} tujuan</span>
                            </div>
                            <div class="mt-4 space-y-2">
                                @foreach($destinations as $index => $destination)
                                    <div class="flex items-center gap-3">
                                        <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-[10px] font-black text-slate-500">{{ $index + 1 }}</span>
                            ...
                            <div class="mt-5 flex items-center justify-between gap-4 border-t border-slate-100 pt-4">
                                <p class="text-xs text-slate-400">{{ $rute->created_at->format('d M Y') }}</p>
                                <form action="{{ route('admin.route.destroy', $rute->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus rute ini?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="rounded-lg bg-red-50 px-4 py-2 text-xs font-bold text-red-500">Hapus</button>
                                </form>
                            </div>
                        </div>
                    @empty
                        <div class="px-6 py-14 text-center text-sm text-slate-400">Belum ada rute perjalanan.</div>
                    @endforelse
                </div>

                {{-- SEARCH EMPTY --}}
                <div id="searchEmpty" class="hidden border-t border-slate-100 px-6 py-12 text-center">
                    <p class="font-bold text-slate-700">Rute tidak ditemukan</p>
                    <p class="mt-1 text-sm text-slate-400">Coba cari menggunakan nama kota lain.</p>
                </div>
            </section>

            {{-- FOOTER --}}
            <footer class="mt-10 flex flex-col gap-2 border-t border-slate-200 py-6 text-xs text-slate-400 sm:flex-row sm:justify-between">
                <p>© {{ date('Y') }} VikensaTrans.</p>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const desktopRows = document.querySelectorAll('.route-row');
        const mobileRows = document.querySelectorAll('.route-mobile');
