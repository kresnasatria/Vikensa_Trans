...
<head>
    ...
    <meta name="description" content="Form Pemesanan VikensaTrans">
    ...
    <style>
        ...
        .booking-card { box-shadow: 0 20px 60px rgba(15, 23, 42, .055); }
        .vehicle-image { transition: transform .5s ease; }
        .vehicle-wrapper:hover .vehicle-image { transform: scale(1.04); }
...
@php
    ...
    $shuttleName = strtolower($schedule->shuttle->name ?? '');

    /* FOTO ARMADA */
    if (str_contains($shuttleName, 'wuling')) {
        $vehicleImage = 'images/wuling.PNG';
    } elseif (($schedule->shuttle->id ?? 1) % 2 === 0) {
        $vehicleImage = 'images/v02.jpeg';
    } else {
        $vehicleImage = 'images/v01.jpeg';
    }
@endphp
...
<aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-50 flex w-[280px] flex-col border-r border-white/10 bg-slate-950 text-white transition-transform duration-300 lg:translate-x-0">
    ...
    <nav class="flex-1 overflow-y-auto px-4 py-6">
        ...
        <div class="mt-9 rounded-3xl border border-white/10 bg-white/[.04] p-5">
            ...
            <p class="mt-4 text-sm font-black text-white">Charter VikensaTrans</p>
            <p class="mt-2 text-xs leading-6 text-slate-500">Tentukan sendiri kota jemput, tujuan, serta waktu perjalanan sesuai kebutuhanmu.</p>
        </div>
    </nav>
        ...
        <div class="border-t border-white/10 p-4">
            <div class="relative">
                <button @click="profileOpen = !profileOpen" type="button" class="flex w-full items-center gap-3 rounded-2xl p-3 text-left transition hover:bg-white/5">
                    ...
                    <svg :class="profileOpen ? 'rotate-180' : ''" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="h-4 w-4 text-slate-500 transition">
                        <path d="m6 9 6 6 6-6"/>
                    </svg>
                </button>
                ...
                <div x-show="profileOpen" x-cloak x-transition @click.outside="profileOpen = false" class="absolute bottom-full left-0 right-0 mb-2 overflow-hidden rounded-2xl border border-slate-200 bg-white p-2 shadow-2xl">
                    <a href="{{ route('profile.edit') }}" class="block rounded-xl px-4 py-3 text-sm font-semibold text-slate-600 transition hover:bg-slate-50">
                        Edit Profil
                    </a>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="block w-full rounded-xl px-4 py-3 text-left text-sm font-semibold text-red-500 transition hover:bg-red-50">
                            Keluar
                        </button>
                    </form>
                </div>
        ...
        <div class="flex w-full items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <h2 class="text-lg font-black text-slate-950">Pemesanan</h2>
                </div>
            </div>
            <a href="{{ route('dashboard') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-bold text-slate-600 transition hover:border-sky-200 hover:bg-sky-50 hover:text-sky-600">
        ...
        <div class="mx-auto max-w-7xl">
            ...
            <div class="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
                <div>
                    ...
                    <h1 class="mt-4 text-3xl font-black tracking-tight text-slate-950 sm:text-4xl">Atur perjalananmu.</h1>
                    <p class="mt-3 max-w-2xl text-sm leading-7 text-slate-500 sm:text-base">Lengkapi informasi pemesan, tentukan kota jemput dan tujuan, lalu pilih waktu perjalanan.</p>
                </div>
                <div class="inline-flex w-fit items-center gap-2 rounded-full bg-white px-4 py-2 text-xs font-bold text-slate-500 shadow-sm ring-1 ring-slate-200">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="h-4 w-4 text-emerald-500">
            ...
            @if($errors->any())
                <div class="mt-7 rounded-2xl border border-red-200 bg-red-50 p-5">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="font-black text-red-700">Data pemesanan belum lengkap</p>
                            <p class="mt-1 text-sm text-red-600">Periksa kembali data berikut:</p>
            ...
            {{-- FORM --}}
            <form action="{{ route('book.store') }}" method="POST" id="bookingForm" class="mt-8">
                @csrf

                {{-- BACKEND DATA --}}
                <input type="hidden" name="schedule_id" value="{{ $schedule->id }}">
                <input type="hidden" name="custom_origin" id="hidden_origin" value="{{ old('custom_origin') }}">
                <input type="hidden" name="custom_destination" id="hidden_destination" value="{{ old('custom_destination') }}">
                ...
                <div class="grid gap-7 xl:grid-cols-[minmax(0,1fr)_380px]">
                    ...
                    <div class="space-y-7">
                        ...
                        <section class="booking-card overflow-hidden rounded-[2rem] border border-slate-200 bg-white">
                            <div class="flex items-center gap-4 border-b border-slate-100 px-6 py-5 sm:px-7">
                                <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-sky-500 text-sm font-black text-white shadow-lg shadow-sky-500/20">01</div>
                                <div>
                                    <h2 class="text-lg font-black text-slate-950">Informasi Pemesan</h2>
                                    <p class="mt-1 text-xs text-slate-400">Data yang dapat dihubungi terkait perjalanan.</p>
                                </div>
                            </div>
                            <div class="grid gap-5 p-6 sm:p-7 md:grid-cols-2">
                                {{-- NAME --}}
                                <div>
                                    <label for="booker_name" class="block text-sm font-bold text-slate-700">Nama Pemesan <span class="text-red-500">*</span></label>
                                    <div class="relative mt-2">
                                        ...
                                        <input id="booker_name" type="text" name="booker_name" value="{{ old('booker_name', Auth::user()?->name ?? '') }}" required placeholder="Nama lengkap pemesan" class="block h-14 w-full rounded-2xl border border-slate-200 bg-slate-50 py-3 pl-12 pr-4 text-sm font-medium text-slate-900 outline-none transition placeholder:text-slate-400 hover:border-slate-300 focus:border-sky-500 focus:bg-white focus:ring-4 focus:ring-sky-500/10">
                                    </div>
                                    @error('booker_name') <p class="mt-2 text-xs font-semibold text-red-500">{{ $message }}</p> @enderror
                                </div>
                                {{-- PHONE --}}
                                <div>
                                    <label for="phone_number" class="block text-sm font-bold text-slate-700">No. WhatsApp / HP <span class="text-red-500">*</span></label>
                                    <div class="relative mt-2">
                                        <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4 text-slate-400">
                                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="h-5 w-5">
                                        ...
                                        <input id="phone_number" type="text" inputmode="tel" name="phone_number" value="{{ old('phone_number') }}" maxlength="20" required placeholder="Contoh: 081234567890" class="block h-14 w-full rounded-2xl border border-slate-200 bg-slate-50 py-3 pl-12 pr-4 text-sm font-medium text-slate-900 outline-none transition placeholder:text-slate-400 hover:border-slate-300 focus:border-sky-500 focus:bg-white focus:ring-4 focus:ring-sky-500/10">
                                    </div>
                                    @error('phone_number') <p class="mt-2 text-xs font-semibold text-red-500">{{ $message }}</p> @enderror
                                </div>
                                {{-- PICKUP ADDRESS --}}
                                <div class="md:col-span-2">
                                    <label for="pickup_address" class="block text-sm font-bold text-slate-700">Alamat Detail Titik Jemput <span class="text-red-500">*</span></label>
                                    <div class="relative mt-2">
                                        <div class="pointer-events-none absolute inset-y-0 left-4 top-4 text-slate-400">
                                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="h-5 w-5">
                                                <path d="M12 22s7-5 7-12a7 7 0 1 0-14 0c0 7 7 12 7 12Z"/>
                                                <circle cx="12" cy="10" r="2"/>
                                            </svg>
                                        </div>
                                        <textarea id="pickup_address" name="pickup_address" rows="4" required placeholder="Contoh: Jl. Ahmad Yani No. 20, depan minimarket, pagar warna hitam..." class="block w-full resize-none rounded-2xl border border-slate-200 bg-slate-50 py-4 pl-12 pr-4 text-sm font-medium leading-6 text-slate-900 outline-none transition placeholder:text-slate-400 hover:border-slate-300 focus:border-sky-500 focus:bg-white focus:ring-4 focus:ring-sky-500/10">{{ old('pickup_address') }}</textarea>
                                    </div>
                                    <p class="mt-2 text-xs leading-5 text-slate-400">Tambahkan nama jalan, nomor rumah, RT/RW, hotel, atau patokan yang jelas.</p>
                                    @error('pickup_address') <p class="mt-2 text-xs font-semibold text-red-500">{{ $message }}</p> @enderror
                                </div>
                            </div>
                        </section>
                        ...
                        <section class="booking-card overflow-hidden rounded-[2rem] border border-slate-200 bg-white">
                            <div class="flex items-center gap-4 border-b border-slate-100 px-6 py-5 sm:px-7">
                                <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-sky-500 text-sm font-black text-white shadow-lg shadow-sky-500/20">02</div>
                                <div>
                                    <h2 class="text-lg font-black text-slate-950">Rute Perjalanan</h2>
                                    <p class="mt-1 text-xs text-slate-400">Pilih kota jemput dan kota tujuan.</p>
                                </div>
                            </div>
                            <div class="grid gap-5 p-6 sm:p-7 md:grid-cols-2">
                                {{-- ORIGIN --}}
                                <div>
                                    <label for="origin_city" class="block text-sm font-bold text-slate-700">Kota Jemput <span class="text-red-500">*</span></label>
                                ...
                                <div class="md:col-span-2 rounded-2xl border border-sky-100 bg-sky-50 p-5">
                                    <div class="flex items-start gap-4">
                                        <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white text-sky-600 shadow-sm">
                                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="h-5 w-5">
                                                <path d="M5 19c0-7 14-7 14-14"/>
                                                <circle cx="5" cy="19" r="2"/>
                                                <circle cx="19" cy="5" r="2"/>
                                            </svg>
                                        </div>
                                        <div>
                                            <p class="text-sm font-black text-sky-900">Rute charter fleksibel</p>
                                            <p class="mt-1 text-xs leading-6 text-sky-700">Kota yang tersedia mengikuti data rute yang sudah dikelola administrator VikensaTrans.</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <section class="booking-card overflow-hidden rounded-[2rem] border border-slate-200 bg-white">
                            <div class="flex items-center gap-4 border-b border-slate-100 px-6 py-5 sm:px-7">
                                <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-sky-500 text-sm font-black text-white shadow-lg shadow-sky-500/20">03</div>
                                <div>
                                    <h2 class="text-lg font-black text-slate-950">Waktu Perjalanan</h2>
                                    <p class="mt-1 text-xs text-slate-400">Tentukan waktu mulai dan selesai sewa.</p>
                                </div>
                            </div>
                            <div class="grid gap-5 p-6 sm:p-7 md:grid-cols-2">
                    ...
                    <div>
                        <div class="sticky top-28 space-y-5">
                            ...
                            {{-- VEHICLE INFO --}}
                            <article class="booking-card overflow-hidden rounded-[2rem] border border-slate-200 bg-white">
                                <div class="vehicle-wrapper relative h-[220px] overflow-hidden bg-slate-200">
                                    <img src="{{ asset($vehicleImage) }}" alt="{{ $schedule->shuttle->name }}" class="vehicle-image h-full w-full object-cover">
                                    <div class="absolute inset-0 bg-gradient-to-t from-slate-950/85 via-slate-950/10 to-transparent"></div>
                                    <div class="absolute left-4 top-4 inline-flex items-center gap-2 rounded-full bg-emerald-500 px-3 py-2 text-[10px] font-black uppercase tracking-wider text-white shadow-lg">
                                        <span class="h-2 w-2 rounded-full bg-white"></span>
                                        Tersedia
                                    </div>
                                    <div class="absolute bottom-5 left-5 right-5">
                                        <p class="text-[10px] font-black uppercase tracking-[.15em] text-sky-300">Armada Pilihan</p>
                                        <h3 class="mt-1 text-2xl font-black text-white">{{ $schedule->shuttle->name }}</h3>
                                        <p class="mt-1 text-xs text-slate-300">{{ $schedule->shuttle->license_plate }}</p>
                                    </div>
                                </div>
                                <div class="p-6">
                                    <div class="flex items-center justify-between gap-4">
                                        <div>
                                            <p class="text-xs font-semibold text-slate-400">Kapasitas</p>
                                            <p class="mt-1 text-sm font-black text-slate-900">{{ $schedule->shuttle->seat_capacity }} Penumpang</p>
                                        </div>
                                        <div class="text-right">
                                            <p class="text-xs font-semibold text-slate-400">Sewa / Hari</p>
                                            <p id="label_base_price" data-price="{{ $schedule->price }}" class="mt-1 text-lg font-black text-sky-600">
                            ...
                            {{-- PRICE SUMMARY --}}
                            <section class="overflow-hidden rounded-[2rem] bg-slate-950 p-6 text-white shadow-xl">
                                <div class="flex items-center justify-between gap-4">
                                    <div>
                                        <p class="text-xs font-black uppercase tracking-[.15em] text-sky-400">Ringkasan Biaya</p>
                                        <h3 class="mt-1 text-lg font-black">Estimasi Pemesanan</h3>
                                    </div>
                                    <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-white/10 text-sky-400">
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="h-5 w-5">
                                            <path d="M12 2v20"/>
                                ...
                                <div class="mt-6 space-y-4">
                                    <div class="flex items-center justify-between gap-4 text-sm">
                                        <span class="text-slate-400">Harga per hari</span>
                                        <span class="font-bold text-white">Rp {{ number_format($schedule->price, 0, ',', '.') }}</span>
                                    </div>
                                    <div class="flex items-center justify-between gap-4 text-sm">
                                        <span class="text-slate-400">Durasi sewa</span>
                                        <span id="label_days_count" class="rounded-lg bg-amber-400/10 px-3 py-1.5 text-xs font-black text-amber-300">1 Hari</span>
                                    </div>
                                </div>
                                <div class="mt-6 border-t border-white/10 pt-5">
                                    <p class="text-xs font-semibold text-slate-400">Total Pembayaran</p>
                                    <p id="label_total_price" class="mt-2 text-3xl font-black tracking-tight text-sky-400">
                                        Rp {{ number_format($schedule->price, 0, ',', '.') }}
                                    </p>
                                    ...
                                    <div class="mt-4 flex items-start gap-3 rounded-xl bg-sky-500/10 px-4 py-3">
                                        ...
                                        <p class="text-[11px] font-medium leading-relaxed text-sky-300">
                                            Sudah termasuk biaya rute perjalanan. Total akhir akan diverifikasi ulang oleh sistem saat pesanan dibuat.
                                        </p>
                                    </div>
                                </div>
                            </section>
                            ...
                            <section class="rounded-[2rem] border border-slate-200 bg-white p-5">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div>
                                        <p class="text-sm font-black text-slate-900">Periksa sebelum memesan</p>
                                        <p class="mt-1 text-xs leading-6 text-slate-500">Pastikan nomor WhatsApp, titik jemput, tujuan, serta waktu perjalanan sudah benar.</p>
                                    </div>
                                </div>
                                @auth
                                    <button type="submit" class="group mt-5 flex w-full items-center justify-center gap-3 rounded-2xl bg-sky-500 px-5 py-4 text-sm font-black text-white shadow-xl shadow-sky-500/20 transition duration-300 hover:-translate-y-0.5 hover:bg-sky-400 focus:outline-none focus:ring-4 focus:ring-sky-500/20">
                                        Konfirmasi & Pesan
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="h-5 w-5 transition group-hover:translate-x-1">
                                            <path d="M5 12h14"/>
                                            <path d="m13 6 6 6-6 6"/>
                                        </svg>
                                    </button>
                                @else
                                    <a href="{{ route('login') }}" class="mt-5 flex w-full items-center justify-center rounded-2xl bg-sky-500 px-5 py-4 text-sm font-black text-white transition hover:bg-sky-400">
                                        Login untuk Melanjutkan
                                    </a>
                                @endauth
                                <a href="{{ route('dashboard') }}" class="mt-3 flex w-full items-center justify-center rounded-2xl border border-slate-200 px-5 py-4 text-sm font-bold text-slate-500 transition hover:bg-slate-50 hover:text-slate-900">
                                    Pilih Armada Lain
                                </a>
                            </section>
                        </div>
...
<script>
    $(document).ready(function () {
        ...
        const oldOrigin = @json(old('custom_origin'));
        const oldDestination = @json(old('custom_destination'));
        ...
        // AMBIL BASE PRICE DARI ID
        const basePrice = parseInt($('#label_base_price').data('price')) || 0;
        let totalDays = 1;

        /* SELECT2 INITIALIZATION */
        originSelect.select2({ width: '100%', placeholder: 'Pilih kota jemput', allowClear: true });
        destinationSelect.select2({ width: '100%', placeholder: 'Pilih kota tujuan', allowClear: true });
        ...
        /* POPULATE ORIGINS */
        const uniqueOrigins = [...new Set(routesData.map(item => item.origin).filter(Boolean))];
        uniqueOrigins.forEach(function (origin) {
            originSelect.append(new Option(origin, origin));
        });

        /* POPULATE DESTINATIONS */
        function populateDestinations(selectedOrigin, selectedDestination = null) {
            destinationSelect.empty().append(new Option('Pilih kota tujuan', ''));
            if (!selectedOrigin) {
            ...
            availableDestinations.forEach(function (route) {
                const destination = formatDestination(route.destination);
                destinationSelect.append(new Option(destination, destination));
            });
        ...
        /* OLD INPUT HANDLER */
        if (oldOrigin) {
            originSelect.val(oldOrigin).trigger('change.select2');
            hiddenOrigin.val(oldOrigin);
            populateDestinations(oldOrigin, oldDestination);
            if (oldDestination) hiddenDestination.val(oldDestination);
        }
        ...
        /* ========================================= */
        /* UPDATE TOTAL PRICE (LOGIKA BARU BIAYA RUTE) */
        /* ========================================= */
        function updateTotalPrice() {
            const selectedOrigin = hiddenOrigin.val();
            const selectedDestination = hiddenDestination.val();
            let biayaTambahanRute = 0;

            if (selectedOrigin && selectedDestination) {
                const ruteTerpilih = routesData.find(function(r) {
                    return r.origin === selectedOrigin && formatDestination(r.destination) === selectedDestination;
                });

                if (ruteTerpilih && ruteTerpilih.biaya) {
                    biayaTambahanRute = parseInt(ruteTerpilih.biaya) || 0;
                }
            }

            const totalAkhir = (basePrice * totalDays) + biayaTambahanRute;
            hiddenTotalPrice.val(totalAkhir);
        ...
        function calculateDays() {
            ...
            if (departureValue && arrivalValue) {
                ...
                if (arrivalDate > departureDate) {
                    ...
                    totalDays = Math.ceil(difference / dayInMilliseconds);
                    totalDays = Math.max(1, totalDays);
                }
            }

            labelDaysCount.text(totalDays + (totalDays === 1 ? ' Hari' : ' Hari'));
            updateTotalPrice();
        }
